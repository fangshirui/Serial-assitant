...
from load_data_new import load_dataset, IMAGE_SIZE
import cv2
...
class Dataset:
    ...
    def load(self,
        ...
        # 这部分代码就是根据 keras 库要求的维度顺序重组训练数据集
        ...
        # 类别标签进行one-hot编码使其向量化，在这里我们的类别有３种，经过转化后标签数据变为三维
...
class Model:
    ...
    def build_model(self, dataset, nb_classes=3):
        ...
        self.model.add(Dense(100))  # 14 Dense层,又被称作全连接层
    ...
    def train(self,
        ...
        sgd = SGD(lr=0.01)  # 采用SGD+momentum的优化器进行训练，首先生成一个优化器对象
    ...
    MODEL_PATH = './road_classfier.model.h5'
    ...
    def road_predict(self, image):
        ...
        # 给出输入属于各个类别的概率，则该函数会给出输入图像属于0和1，2的概率各为多少
...
if __name__ == '__main__':
    road_data_set = Dataset('./dataset')
    road_data_set.load()

    model = Model()
    model.build_model(road_data_set)

    model.train(road_data_set,nb_epoch=30)
    model.save_model(file_path='./road_classfier.model.h5')
    model.evaluate(road_data_set)
